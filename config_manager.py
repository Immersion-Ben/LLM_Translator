...
def _ascii_safe_dir(path: Path) -> Path:
    """Tesseract C++ 엔진은 Windows 에서 비ASCII(한글 등) 경로의 tessdata/입력을
    열지 못한다(UnicodeDecodeError / "does not exist"). 배포 폴더명이 한글이어도
    OCR 이 동작하도록, 비ASCII 경로를 ASCII 경로로 변환한다:
    8.3 short path 우선, 안 되면 ASCII 위치(PUBLIC/ProgramData)에 junction 을 만든다."""
    s = str(path)
    ...
    for base in (os.environ.get("PUBLIC"), os.environ.get("ProgramData")):
        if not base or not base.isascii():
            continue
        link = Path(base) / ".llm_translator" / "tess"
        try:
            ...
            continue
    logger.warning("TESS: 비ASCII 경로를 ASCII 로 변환하지 못함 — OCR 실패 가능: %s", s)
    return path
...
class ConfigManager:
    ...
    def apply_tesseract_path(self) -> None:
        ...
        if tess_path and os.path.exists(tess_path):
            # 비ASCII(한글) 배포 경로에서도 OCR 이 되도록 ASCII 경로로 보정한다.
            tess_dir = _ascii_safe_dir(Path(tess_path).parent)
            safe_cmd = tess_dir / "tesseract.exe"
            pytesseract.pytesseract.tesseract_cmd = str(safe_cmd if safe_cmd.exists() else tess_path)
